...
from courses_apps.classroom.selectors import get_classroom_details
from django.contrib.auth import get_user_model

User = get_user_model()

class GetClassRoomDetailsAPIViewTestCase(APITestCase):
    def setUp(self):
        self.client = APIClient()
        self.get_classroom_details_uri = reverse('classroom:classroom_details')
    ...
    def test_get_classroom_details_authenticated_teacher(self):
        """Test that an authenticated teacher can get the details of a classroom."""
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.teacher_access_token)
        response = self.client.post(self.get_classroom_details_uri, {'class_code': 'TC123'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['success'], True)
        self.assertEqual(response.data['message'], "Classroom details fetched successfully.")
        self.assertEqual(response.data['data']['title'], "Test Classroom")
        self.assertEqual(response.data['data']['class_code'], "TC123")
        self.assertEqual(response.data['data']['student_count'], 1)

    def test_get_classroom_details_unauthenticated_user(self):
        """Test that an unauthenticated user cannot get the details of a classroom."""
        response = self.client.post(self.get_classroom_details_uri, {'class_code': 'TC123'})
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_get_classroom_details_authenticated_learner(self):
        """Test that an authenticated learner cannot get the details of a classroom."""
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.learner_access_token)
        response = self.client.post(self.get_classroom_details_uri, {'class_code': 'TC123'})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_get_classroom_details_invalid_token(self):
        """Test that an invalid token results in authentication failure."""
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + 'invalid_token')
        response = self.client.post(self.get_classroom_details_uri, {'class_code': 'TC123'})
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_get_classroom_details_nonexistent_class_code(self):
        """Test that a request for a nonexistent class code returns a 400 error."""
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.teacher_access_token)
        response = self.client.post(self.get_classroom_details_uri, {'class_code': 'NONEXISTENT'})
